...
\begin{abstract}
   This project consists on an implementation of a Load Balancer and Auto-Scaler for
   a program running on Amazon Web Services that searches for the maximum value 
   of a map using different search strategies (A*, BFS, DFS). It's objective is to 
   produce metrics from running replics and adapt the system to get the maximum value
   of the instaces runinng in AWS.
\end{abstract}

%------------------------------------------------------------------------- 
\Section{Metrics}

We used BIT (Bytecode Instrumenting Tool) to allow us to alter the Web Server 
Java Class Byte Code so it could measure the metrics that we wanted in a way 
that would help us decice de the cost of replying to a request.\\
This metrics need to be heavily weighted as they might give an overhead to the 
the original program. For exampe: the metric instructions executed is a bad metric
as it will had extra intructions to count every instruction which will lead to a 
big overhead.
%-------------------------------------------------------------------------
...
file so that we could later retrive and use them on our Load Balancer. Each 
request has its own file with its metrics and it's named by its thread id 
and the UNIX time of the request.

%------------------------------------------------------------------------- 
\Section{Load Balancer and Auto Scaler}

For this checkpoint we made use of a classic Load Balancer of Amazon Web
Services along with a Auto Scaling Group to manage the running instances and
to allow a scale mechanism while we don't implement our own Load Balancer and 
Auto Scaler.  
...
We created an Auto Scaling Group that increases the number of intances by
